...
public class OfficeReadPlatformServiceImpl implements OfficeReadPlatformService {
	...
	private final static Logger logger =  LoggerFactory.getLogger(OfficeReadPlatformServiceImpl.class);
	...
	private static final class OfficeMapper implements RowMapper<OfficeData> {
		...
		public String officeSchema() {
			logger.info("companyName  =  mapRow==cin==============**************************************Aiswarya=");
			return " o.id as id, o.name as name, " + nameDecoratedBaseOnHierarchy
					+ " as nameDecorated, o.external_id as externalId, o.opening_date as openingDate, "
					+"o.cin as cin, o.company_name as company_name,o.company_status as o.company_status , o.roc as roc, "
					+ "o.incorportated_date as incorportated_date, o.funds as funds, o.registration_address as registration_address,"
		...
		public OfficeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
			...
			/*
			 * Mapping new fiels for office on Aug 28,2019 by Aiswarya
			 */
			final String cin = rs.getString("cin");
			final String roc = rs.getString("roc");
			final String companyName = rs.getString("companyName");
			final String companyStatus = rs.getString("companyStatus");
			final Integer funds = rs.getInt("funds");
			final Integer registrationNumber = rs.getInt("registrationNumber");
			final LocalDate incorporatedDate = JdbcSupport.getLocalDate(rs, "incorporatedDate");
			final String registrationAddress = rs.getString("registrationAddress");
			logger.info(companyName+"companyName  =  mapRow==cin==============**************************************Aiswarya="+cin);
			return new OfficeData(id, name, nameDecorated, externalId, openingDate, hierarchy, parentId, parentName,
					null, cin, roc, companyName, companyStatus, registrationNumber, registrationAddress, funds,
					incorporatedDate);
		}
	}
	...
	public Collection<OfficeData> retrieveAllOffices(final boolean includeAllOffices,
		...
		logger.info(includeAllOffices+"includeAllOffices  =  retrieveAllOffices==cin==============**************************************=");
		...
		logger.info(sqlBuilder+"sqlBuilder  =  retrieveAllOffices==cin==============**************************************=");
